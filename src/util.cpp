...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Sbrc";
#else
    fs::path pathRet;
...
{
    fs::path pathConfigFile(GetArg("-conf", "sbrc.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty sbrc.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override sbrc.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", "sbrcd.pid"));
