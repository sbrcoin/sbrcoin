...

    {
        ...
        if (hasZcSpendInputs)
        {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : zSBRC spend tx %s already in block %d",
        ...
        else
        {
            ...
            // Reject legacy zSBRC mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zSBRC mint output in tx %s",
                                           __func__, tx.GetHash().GetHex()),
...
bool DisconnectBlock(CBlock &block, CValidationState &state, CBlockIndex *pindex, CCoinsViewCache &view, bool *pfClean)
{
    ...
    //Track zSBRC money supply
    if (!UpdateZPIVSupplyDisconnect(block, pindex))
        return error("%s: Failed to calculate new zSBRC supply", __func__);
...
bool ConnectBlock(const CBlock &block, CValidationState &state, CBlockIndex *pindex, CCoinsViewCache &view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    {
        return state.DoS(100, error("%s: Failed to calculate new zSBRC supply for block=%s height=%d", __func__, block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
    }

    // A one-time event where the zSBRC supply was off (due to serial duplication off-chain on main net)
...
bool CheckBlock(const CBlock &block, CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction &tx : block.vtx)
    {
        ...
        // double check that there are no double spent zSBRC spends in this block
        if (tx.HasZerocoinSpendInputs())
        {
            for (const CTxIn &txIn : tx.vin)
            {
                ...
                if (txIn.IsZerocoinSpend() || isPublicSpend)
                {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSBRC serial %s in block\n Block: %s",
