    ...
    model(0)
{
    ...
    // Title
    ui->labelTitle->setText(tr("Select zSBRC Denominations to Spend"));
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < nRequiredConfs)
                ...
                strReason = "Your wallet is locked. Impossible to spend zSBRC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zSBRC seed used to mint this zSBRC is not the same as currently hold in the wallet";
            else
