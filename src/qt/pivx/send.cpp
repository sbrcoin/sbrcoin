    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer SBRC"));
    setCssProperty(ui->labelSubtitle1, "text-subtitle");

    /* Address */
    ui->labelSubtitleAddress->setText(tr("SBRC address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 SBRC");
...
void SendWidget::refreshView()
{
    ...
    ui->pushButtonSave->setText(tr("Send SBRC"));
...
void SendWidget::showHideCheckBoxDelegations()
{
    ...
    // coincontrol is not selected, and we are trying to spend SBRC (not zSBRC)
...
void SendWidget::onCoinControlClicked()
{
    if (walletModel->getBalance() > 0) {
        ...
        inform(tr("You don't have any SBRC to select."));
    }
}
