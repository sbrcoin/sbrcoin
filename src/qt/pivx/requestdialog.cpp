    ...
    ui(new Ui::RequestDialog)
{
    ...
    ui->labelMessage->setText(tr("Instead of sharing only a SBRC address, you can create a payment request, bundling up more information."));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 SBRC");
...
void RequestDialog::onNextClicked(){
    if(walletModel) {
        ...
        if (this->isPaymentRequest) {
            ...
            title = "Request for " + BitcoinUnits::format(displayUnit, value, false, BitcoinUnits::separatorAlways) + " SBRC";
        } else {
