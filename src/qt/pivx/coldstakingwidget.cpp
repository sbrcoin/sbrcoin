    ...
    isLoading(false)
{
    ...
    ui->labelSubtitle2->setText(tr("Accept SBRC delegation / Delegate SBRC"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select SBRC outputs to delegate.");
...
void ColdStakingWidget::updateStakingTotalLabel()
{
    ...
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 SBRC" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}
