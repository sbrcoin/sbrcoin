        ...
        std::list<std::pair<CBitcoinAddress*,CAmount>> addressesTo, CBitcoinAddress* changeAddress)
{
    ...
    receipt.SetStatus("Spend Successful", ZPIV_SPEND_OKAY);  // When we reach this point spending zSBRC was successful
        ...
        CBitcoinAddress* changeAddress)
{
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zSBRC"), nStatus);
        return false;
    }
    ...
    const int nMaxSpends = Params().GetConsensus().ZC_MaxPublicSpendsPerTx; // Maximum possible spends for one zSBRC public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zSBRC used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        ...
        // Select the zSBRC mints to use in this spend
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zSBRC in the future, for now it's useless,
